...
# --- stage3: ノード（交差点）インターロック用の現在占有ノード（有限ノード容量のときだけ使う） ---
# _held_node: この搬送者が今占有している制御点 id（""＝未占有）。デッドロック回避方針:
...
func _congestion_active() -> bool:
	return pool != null and pool.network != null \
		and (pool.network.has_finite_edge_capacity() \
			or pool.network.has_finite_node_capacity())

## ノード（交差点）インターロックを起動する条件。有限ノード容量が1つでもあるときだけ true。
## false のとき（＝全ノード INF＝既定）は request_node/finish_node を一切呼ばず、辺のみを
## 予約する従来の輻輳走行と完全に同一（占有は増えず、イベント順・rng も不変＝バイト一致）。
func _node_interlock_active() -> bool:
	return pool != null and pool.network != null \
		and pool.network.has_finite_node_capacity()
...
func _begin_congested_leg(target: Vector3, st: String, on_arrive: Callable) -> void:
	...
	# 各区間の "enter" は「その区間の終端で進入する制御点 id」（ノードインターロック用）。
	# 接近区間は最初の制御点へ、辺区間は辺の到達側ノードへ進入する。離脱区間／直行は ""（無し）。
	_cong_segs = []
	if route.is_empty():
		# ネットワーク到達不能：直行フォールバック（保存則を最優先）。
		_cong_segs.append({"to": target, "edge": null, "from": "", "enter": ""})
	else:
		# 接近区間: 現在地 → 最初の制御点（辺予約なし、進入前に最初のノードを予約）。
		_cong_segs.append({"to": net.nodes[route[0]], "edge": null, "from": "", "enter": String(route[0])})
		# 各辺の横断区間（進入前に辺を予約、渡り切ってから到達側ノードを予約）。
		for i in range(route.size() - 1):
			var a: String = String(route[i])
			var b: String = String(route[i + 1])
			_cong_segs.append({"to": net.nodes[b], "edge": [a, b], "from": a, "enter": b})
		# 離脱区間: 最後の制御点 → target（辺予約なし、最後のノードを解放してから移動）。
		_cong_segs.append({"to": target, "edge": null, "from": "", "enter": ""})
	_cong_idx = 0
	_run_cong_segment()
...
func _run_cong_segment() -> void:
	...
	if seg["edge"] == null:
		if _node_interlock_active():
			var enter: String = String(seg.get("enter", ""))
			if enter != "" and _held_node == "":
				# 接近区間: ネットワーク最初の制御点へ入る前にノードを予約する（占有上限の先取り）。
				# 満杯なら pickup 側で待機（辺は保持していないのでネットワーク入口での待ち）。
				if pool.network.request_node(enter, self, _on_approach_node_granted):
					_held_node = enter
					_move_cong_free(seg["to"])
				# else: 待機。空けば network が _on_approach_node_granted を呼ぶ。
				return
			elif enter == "" and _held_node != "":
				# 離脱区間: 最後の制御点を「過ぎ去った」とみなし解放してから target へ移動。
				pool.network.finish_node(_held_node, self)
				_held_node = ""
				_move_cong_free(seg["to"])
				return
		_move_cong_free(seg["to"])
...
## ノードインターロック方式は「ブロック区間」型: 元ノード a は横断中ずっと保持し続け、辺 a→b を
## 渡り切って b の占有を確定できた瞬間に a と辺を同時解放する（＝交差点 a を1車ずつ通す）。
## これによりノード容量が実効的に交通を律速する（保持時間が辺横断分の非零になる）。
...
## 辺の横断完了。ノードインターロック有効時は【辺を保持したまま】到達側ノードの進入を要求し、
## 満杯なら待機（＝スピルバック：辺が空かないので上流がバックアップする）。空いていれば辺を
## 解放して前進する。インターロック無効時は従来どおり即座に辺を解放して次区間へ進む。
func _on_cong_edge_crossed() -> void:
	...
	if _node_interlock_active():
		var enter: String = String(seg.get("enter", ""))
		if enter != "":
			# 到達側ノード（交差点）へ進入を要求。満杯なら辺 e を保持したまま FIFO 待機する。
			if pool.network.request_node(enter, self, _on_exit_node_granted):
				_enter_exit_node(seg)
			# else: 待機。辺 e は解放しない（占有継続＝上流バックアップ＝ブロッキングバック）。
			return
	pool.network.finish_edge(e[0], e[1], self, String(seg["from"]))
	_cong_idx += 1
	_run_cong_segment()

## 到達側ノード b の占有を確定できたので、保持していた辺 e と元ノード a(=seg.from) を同時に
## 解放し、以後は b を保持して次区間へ進む。解放時に辺・ノード双方の FIFO 待機者を起床させる。
func _enter_exit_node(seg: Dictionary) -> void:
	var e: Array = seg["edge"]
	pool.network.finish_edge(e[0], e[1], self, String(seg["from"]))
	if _held_node != "":
		pool.network.finish_node(_held_node, self)
	_held_node = String(seg.get("enter", ""))
	_cong_idx += 1
	_run_cong_segment()

## 待機していた到達側ノードが空いてネットワークに占有確定された時のコールバック。
func _on_exit_node_granted() -> void:
	_enter_exit_node(_cong_segs[_cong_idx])

## 待機していた接近ノードが空いて占有確定された時のコールバック（接近移動を開始）。
func _on_approach_node_granted() -> void:
	var seg: Dictionary = _cong_segs[_cong_idx]
	_held_node = String(seg.get("enter", ""))
	_move_cong_free(seg["to"])
