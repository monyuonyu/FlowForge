...
## 入力ポートを持つか（下流としてアイテムを受理できるか）。既定 true。
## Source のような発生源は入力を持たないため override して false を返す。
## Editor.can_connect が「受理不能な先への配線（＝永久ブロック）」を弾く判定に使う。
## 純粋な問い合わせであり sim 経路からは呼ばれない（マーカー値に無影響）。
